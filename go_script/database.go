...
func isUserNew(chat Chat) (bool, error) {
	response, _ := db.Query("SELECT * FROM shop_user WHERE user_id=$1", chat.user_id)
	if !response.Next() {
		_, err := db.Exec("INSERT INTO shop_user VALUES ($1, $2, $3, FALSE)", chat.user_id, chat.user_name, chat.first_name)
		if err != nil {
			return false, err
		}
		return true, nil
...
func updateUserInfo(response *sql.Rows, chat Chat) {
	...
	if user_name != chat.user_name || first_name != chat.first_name {
		for i := 0; i < 10; i++ {
			_, err := db.Exec("UPDATE shop_user SET user_name=$1, first_name=$2 WHERE user_id=$3", chat.user_name, chat.first_name, chat.user_id)
			if err == nil {
				break
			}
...
func isActiveChat(user_id int64) (bool, int) {
	...
	response, _ := db.Query("SELECT is_chat_active, order_id FROM shop_order WHERE user_id_id=$1 AND is_chat_active=TRUE", user_id)
	response.Next()
...
func isOrderActive(user_id int64) bool {
	...
	response, _ := db.Query("SELECT is_order_active FROM shop_user WHERE user_id=$1", user_id)
	response.Next()
...
func makeOrderActive(user_id int64) {
	_, err := db.Exec("UPDATE shop_user SET is_order_active=TRUE WHERE user_id=$1", user_id)
	if err != nil {
...
func makeNewOrder(user_id int64, order_value string) {
	_, err := db.Exec("UPDATE shop_user SET is_order_active=FALSE WHERE user_id=$1", user_id)
	if err != nil {
		go SendMessage(user_id, "Произошла ошибка. Попробуйте еще раз.")
		return
	}
